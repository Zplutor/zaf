...
void PopupMenu::PopupAsSubMenu(const std::shared_ptr<MenuItem>& owner_menu_item) {
    ...
    auto position_in_screen = owner_menu->TransformToScreen(position_in_window);
...
void PopupMenu::DelayShowSubMenu() {
    ...
    show_sub_menu_timer_ = 
        rx::Timer::Once(std::chrono::milliseconds(hover_time), rx::MainThreadScheduler::Instance())
