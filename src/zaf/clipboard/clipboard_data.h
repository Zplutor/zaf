...
public:
    ...
    /**
    Writes the data to a medium according to the specified data descriptor.

    @param data_descriptor
        The data descriptor describing how the data will be written, including the format type to 
        write, and the desired medium types the method returns.
    ...
    @throw zaf::InvalidOperationError
        Thrown if the data cannot be written as specified by the data descriptor. This includes but 
        ...
        - none of the medium types specified in the data descriptor are supported by the data.
    ...
    */
    virtual Medium WriteToMedium(const DataDescriptor& data_descriptor) const = 0;

    /**
    Reads data from the specified medium according to the specified data descriptor.

    @param medium
        The medium from which the data will be read.

    @param data_descriptor
        The data descriptor describing how the data will be read, such as the format type to 
        read.

    @throw zaf::InvalidOperationError
        Thrown if the data cannot be read from the medium as specified by the data descriptor.
        This includes but is not limited to the following situations:
        - the type of the medium is not supported by the data;
        - the format type specified in the data descriptor is not compatible with the data.

    @throw zaf::InvalidDataError
        Thrown if the content to be read in the medium is invalid.
    ...
    @details
        ...
        5. Read the content from the medium in the specified format type and according to other 
           ...
           throw zaf::InvalidDataError.
    */
    virtual void ReadFromMedium(const Medium& medium, const DataDescriptor& data_descriptor) = 0;
};
