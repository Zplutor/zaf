...
public:
    ...
    ~DoAfterTerminateProducer() noexcept {
        DoDisposal();
    }
...
std::shared_ptr<SubscriptionCore> DoAfterTerminateOperator::Subscribe(ObserverShim&& observer) {

    auto producer = std::make_shared<DoAfterTerminateProducer>(
        std::move(observer), 
        after_terminate_);
