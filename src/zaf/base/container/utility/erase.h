...
void Erase(C& container, const typename C::value_type& element) {
    static_assert(!IsAssociativeContainer<C>::Value, "zaf::Erase() can not be used with associative containers.");
	container.erase(std::remove(container.begin(), container.end(), element), container.end());
}

template<typename C, typename P>
void EraseIf(C& container, P predicate) {
    static_assert(!IsAssociativeContainer<C>::Value, "zaf::EraseIf() can not be used with associative containers.");
